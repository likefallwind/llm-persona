#!/usr/bin/env python3
"""Draw the frozen prospective design; this figure contains no outcome data."""
import os
...
def main():
    ...
    ax.text(.5, .97, 'Prospective design: separate sources, predictions saved before generation',
    ...
    box(.02, .565, 'Training answers',
        '32 questions / 30 source families\nFour domains / canonical role\n5 models · 4 states · 2 requests\n1,280 planned answers', '#EAF2F8')
    box(.3525, .565, 'Forecast lock',
        'Fit and tune on training sources\nSave forecasts and fitting choices\nPrimary and sensitivity panels\nBefore confirmation generation', '#F0F1F3')
    box(.685, .565, 'New-source answers',
        '32 separate source families\nSame four domains / canonical role\n5 models · 4 states · 2 requests\n1,280 planned answers', '#E8F3EF')
    ...
    ax.text(.5, .495, 'Three questions evaluated on the confirmation responses',
            ha='center', va='center', fontsize=15, fontweight='bold', color='#203544')
    box(.02, .145, 'RQ1 · Recurring defaults',
        'Repeat agreement and profiles\nTwo requests under identical inputs\nBehavior rates reported alongside\n32 new source families', '#EAF2F8', .285)
    box(.3525, .145, 'RQ2 · Behavior prediction',
        'Four nested forecasts:\nsituation; model defaults;\nmodel × domain; model × state\nLosses averaged by source', '#E8F3EF', .285)
    box(.685, .145, 'RQ3 · Prompt plasticity',
        '16 matched source families\nCanonical; neutral A/B; ASK; EXPLAIN\nCrossed with states and repeats\n2,560 additional answers', '#FAF0E5', .285)
    ax.text(.5, .08, 'Supplementary probes: 160 complete/missing-information answers + 40 sentinel answers.',
