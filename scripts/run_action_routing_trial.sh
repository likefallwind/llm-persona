...
OUT="$ROOT/artifacts/action_routing_trial_v1/run"
STATE="$OUT/run_state"
...
"$PY" "$ROOT/scripts/generate_action_routing_trial.py" \
  --spec "$ROOT/data/action_routing_trial_spec_v1.json" \
  --output-dir "$ROOT/artifacts/action_routing_trial_v1"
code=$?
if [[ "$code" -eq 0 ]]; then
  "$PY" "$ROOT/scripts/run_action_routing_trial.py" \
    --spec "$ROOT/data/action_routing_trial_spec_v1.json" \
    --request-manifest "$OUT/request_manifest.jsonl" \
    --order-plan "$ROOT/artifacts/action_routing_trial_v1/request_order_plan.jsonl" \
    --output-dir "$OUT" \
...
if [[ "$code" -eq 0 ]]; then
  "$PY" "$ROOT/scripts/factorial_prompt_status.py" \
    --spec "$ROOT/data/action_routing_trial_spec_v1.json" \
    --manifest "$ROOT/artifacts/action_routing_trial_v1/sample_manifest.jsonl" \
