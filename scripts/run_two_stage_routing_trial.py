#!/usr/bin/env python3
"""Run the frozen two-stage routing trial in balanced model-specific order."""
...
from generate_two_stage_routing_trial import order_hash, stratum_key
from run_action_routing_trial import call_one
...
def planned_samples(
    ...
    if [int(row["queue_rank"]) for row in model_plan] != list(range(len(samples))):
        raise RuntimeError(f"invalid two-stage queue ranks for {model}")
    if {str(row["sample_id"]) for row in model_plan} != set(sample_map):
        raise RuntimeError(f"two-stage order-plan samples differ for {model}")
    for row in model_plan:
        block_index = int(row["queue_rank"]) // 10
        sample = sample_map[str(row["sample_id"])]
        if int(row.get("block_index", -1)) != block_index:
            raise RuntimeError(f"two-stage block mismatch for {model}|{row['sample_id']}")
        if row.get("stratum_key") != stratum_key(sample):
            raise RuntimeError(f"two-stage stratum mismatch for {model}|{row['sample_id']}")
        expected_hash = order_hash(seed, model, f"block|{block_index}|{row['sample_id']}")
        if row.get("order_sha256") != expected_hash:
            raise RuntimeError(f"two-stage order hash mismatch for {model}|{row['sample_id']}")
...
def main() -> None:
    ...
    parser.add_argument("--request-manifest", type=Path, default=Path("artifacts/two_stage_routing_trial_v1/run/request_manifest.jsonl"))
    parser.add_argument("--order-plan", type=Path, default=Path("artifacts/two_stage_routing_trial_v1/request_order_plan.jsonl"))
    parser.add_argument("--output-dir", type=Path, default=Path("artifacts/two_stage_routing_trial_v1/run"))
    ...
    if len(samples) * len(models) != expected or len(plan) != expected:
        raise RuntimeError("frozen two-stage call count does not match spec")
    for model in models:
        planned_samples(samples, plan, model, seed)
    print(json.dumps({
        "samples": len(samples), "models": models, "expected_calls": expected,
        "order": "model-specific SHA-256 order in exact ten-stratum blocks",
    ...
    try:
        ...
        raise RuntimeError("another writer holds the two-stage output lock") from exc
    ...
    with output_path.open("a", encoding="utf-8") as handle:
        for model in models:
            ...
            def run_sample(sample: dict[str, Any]) -> dict[str, Any]:
                ...
                row["two_stage_order_seed"] = seed
                row["two_stage_queue_rank"] = rank[sample["sample_id"]]
                return row
            ...
            with ThreadPoolExecutor(max_workers=concurrency) as pool:
                ...
                for future in as_completed(futures):
                    ...
                    print(json.dumps({
                        ...
                        "queue_rank": row["two_stage_queue_rank"],
                        "call_type": row["arm"],
