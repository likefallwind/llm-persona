    ...
    for relative, wanted in sorted(needed.items()):
        ...
        for number, material, record in wanted:
            ...
            item = {'sample_id': sid, 'blind_id': blind, 'model': record['model'],
                    ...
                    'response': text}
            # Match the persisted mapping's recursive key order before the
            # conversation object is serialized inside the coder's user text.
            item = json.loads(json.dumps(item, ensure_ascii=False, sort_keys=True))
            mapping.append(item)
            for judge in JUDGES:
