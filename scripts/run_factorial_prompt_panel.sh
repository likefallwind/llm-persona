...
OUT="$ROOT/artifacts/factorial_prompt_v1/run"
STATE="$OUT/run_state"
...
"$PY" "$ROOT/scripts/generate_factorial_prompt_panel.py" \
  --spec "$ROOT/data/factorial_prompt_spec_v1.json" \
  --output-dir "$ROOT/artifacts/factorial_prompt_v1"
code=$?
if [[ "$code" -eq 0 ]]; then
  "$PY" "$ROOT/scripts/run_factorial_prompt_panel.py" \
    --spec "$ROOT/data/factorial_prompt_spec_v1.json" \
