...
\midrule
Recurring differences in help & Giving answers and inviting reasoning; supporting directness ratings & Some helping tendencies distinguish models under the tested conditions\\
Differences in limited settings & Response complexity, expressed confidence, and within-task safety patterns & Consistency depends on the behavior and conditions tested\\
Shared responses & Identical choices on the default affiliation questionnaire & These fixed-choice items do not distinguish the tested models\\
Uncertain consistency & Mixed results for revising or withholding answers; failed screening criteria & Insufficient support for recurrence does not establish absence of differences\\
\bottomrule
\end{tabular}
...
reasoning. On the 32 test problems under the common tutoring instruction, GLM reveals an answer in
...
direction remains the same when each annotator is excluded
...
Differences in expressed confidence recur across four types of context in six
previously tested models (ICC 0.852). This
...
tests on new responses as the three action measures (Appendix~\ref{app:behavior-evidence}).
...
pilot, but affiliation ratings closely overlap with ratings of warmth of the wording.
...
screening rule. Successful prediction using a separate binary measure---
whether a reply invites reasoning---does not validate that scale. We retain
these failed checks without treating them as proof that models never differ. The next experiments predict whether models give answers, invite reasoning,
and acknowledge feelings or difficulty, then test how instructions change those actions. They do not validate
