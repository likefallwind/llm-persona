...
**目前档案提供的证据。** 档案提供广度，也揭示分析单位和历史来源的重要性。导师、评分和考试输出不能合为百万个独立人格观测。离线恢复现已关联六类对话任务的输入与输出，但无法证明每一次历史请求的完整内容。另行冻结的自然对话验证已于 9 月 8 日获授权启动，待完成编码与分析，在本稿中不提供行为证据；构造学生消息能否推广到档案对话，仍未解决。

## 结论

这些部署具有反复出现的教学倾向，但不同规律的迁移边界不同。默认画像能预测新问题行为，其中答案揭示与推理邀请的增益在已测试的学生表达变化下仍保留。承接状态画像虽在原措辞的新问题上有效，却未在新表达下保留额外增益。明确教学指令进一步使要求的动作趋同，同时允许伴随行为不同。因此，可辩护的教育人格主张必须说明哪些动作反复出现、跨哪些输入变化、优于哪个情境基线。结果支持依赖提示的动作画像，不建立不可变的人类式类型、情绪理解或学习收益。在本稿中，向档案自然对话的迁移仍未检验。
...
另行冻结的协议选择 256 个符合条件的精确来源组，配对七个历史模型在脚手架／教学策略两臂下的 3,584 条已有回答，需要 10,752 次自动编码。来源分组交叉拟合和配对指令效应已在编码前规定。该验证已于 9 月 8 日获授权启动；本稿尚无完整验证结果，不报告自然对话行为结论。这些数字描述已恢复数据和计划样本，不属于额外完成的实验证据。
