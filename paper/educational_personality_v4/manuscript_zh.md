...
教育语境使这个问题具有可观察的行为对象。是否揭示答案、是否邀请学生推理，都能从实际回答中编码；两者可以同时出现，也没有哪一种永远更好。回应学生表达的困难同样可以观察，而无需推断模型是否体验同理心。本文研究的是导师模型的行为倾向，与 PATS 等根据学生人格调整教学的研究对象不同 [@rooein2026pats]，也不据此推断教学质量或学习增益。

已有研究通过人格情境和行为探针识别模型差异 [@lee2025trait; @liu2026bma]，教育领域也已比较模型教学动作组合，并从对话中学习导师差异 [@kucheria2025cima; @lee2026tutorpersonas]。本文在此基础上检验默认倾向和学生状态响应的额外预测信息，与情境、模型在不同学科的习惯及简单训练输出风格比较，并在新回答生成前锁定预测。

全文围绕三个相互衔接的问题展开：

1. **可重复差异**：相同教育输入下，不同模型是否表现出可区分且跨重复请求复现的默认倾向？
2. **跨问题与表达预测**：默认倾向或模型特有的学生状态响应，能否预测独立来源新题的行为，并进一步迁移到新的学生表达？
...
首阶段支持三项默认行为的新来源预测，以及承接行为的额外状态增益。观察这些结果后，我们设计新的表达扩展，在新增生成前冻结材料和预测，复用 32 个确认题来区分表达变化与问题变化。默认的揭示／邀请增益跨表达保留，承接状态画像的增量则失效，同期原措辞对照仍保留正增益。结合指令下的动作趋同，研究区分三种常被混用的说法：默认倾向反复出现、学生状态响应能够迁移，以及行为能在明确教学政策下保持。这种经验区分，包括更强迁移主张的失败，构成本文的主线。有限的构造材料限制了外推，也不能从预测成败推断内在心理、情绪理解或学习收益。
...
观察首阶段结果后，我们冻结后续扩展，复用其 32 个问题、原模型预测、五个部署、两种学生作答状态和每单元两次请求。四个表达臂各有 1,280 条回答：同期重生成原始平静／挫败措辞、新的显式同义表达、隐含线索，以及同学情境对照。扩展增加 5,120 条回答，两阶段合计 10,440 条导师生成，不含测量开发试验。这增加的是表达条件，不是独立问题来源；两个阶段的确认标签都未用于重新拟合预测器。
