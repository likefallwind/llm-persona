...
**相对优势与绝对准确性不同。** 事后诊断进一步区分两个表达臂：显式下，承接条件画像 Brier 从 0.145806 略降至 0.144501，但模型—领域基线改善更多，从 0.158289 降至 0.140840；隐含下，条件误差上升至 0.155411，而基线下降至 0.149134。因此两个臂都失去增量优势，只有后者的条件画像绝对预测误差变大。精确分解显示，由于两画像平均概率几乎相同，整体承接率变化对任一增益变化的贡献绝对值都小于 0.000003；其余变化与逐格预测差共变。该描述性记账不能排除更丰富的校准或语义解释，详见新增附录。
...
**目前档案提供的证据。** 档案提供广度，也揭示分析单位和历史来源的重要性。导师、评分和考试输出不能合为百万个独立人格观测。离线恢复现已关联六类对话任务的输入与输出，但无法证明每一次历史请求的完整内容。另行冻结的自然对话验证尚未执行，在本稿中不提供行为证据；构造学生消息能否推广到档案对话，仍未解决。
...
另行冻结的协议选择 256 个符合条件的精确来源组，配对七个历史模型在脚手架／教学策略两臂下的 3,584 条已有回答，需要 10,752 次自动编码。来源分组交叉拟合和配对指令效应已在编码前规定。截至本稿检查点，该验证尚未执行，不报告自然对话行为结果。这些数字描述已恢复数据和计划样本，不属于额外完成的实验证据。
